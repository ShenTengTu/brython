...
def make_doc():
    ...
    src_paths = [
        static_doc_path,
        static_doc_path.joinpath("cookbook"),
    ...
    for lang in languages:
        ...
        for i, (src_path, dest_path) in enumerate(
            # zip([md_doc_path.joinpath(lang), md_doc_path.joinpath(lang, "cookbook")], dest_paths,)
            zip([md_doc_path.joinpath(lang)], dest_paths,)
        ):
            for file_path in src_path.iterdir():
                ...
                if ext == ".md":
                    ...
                    if i == 1:
                        html = html.replace(
                            'class="navig" href="', 'class="navig" href="../'
                        )
                    if scripts:
                        script_content = "\n".join(scripts)
